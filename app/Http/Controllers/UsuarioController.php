...
class UsuarioController extends Controller
{
    ...
    function getLoginUser(Request $request)
    {
        ...
        if ($user && Hash::check($data['cpassword'], $user->cpassword) && strtoupper($user->cestado) == 'A') {
            ...
            $responseData = array(
                "rpta" => 0, "error" => "not content", "message" => "revise su usuario y contraseña",
                "datos" => []
            );
            return response()->json($responseData, 200);
        }
    ...
    //TODO * CREAR FUNCIÓN PARA BUSCAR UNA KEY PERSONA

    function ckeyUpdate($codigoPersona, $key)
    {
        //buscamos persona        
        $persona = User::where('ncodpersona', $codigoPersona)->first();
        //agregamos la nueva key
        $persona->ckeypersona = $key;
        //guardamos
        $persona->save();
        //retornamos
        return response()->json(Utilitarios::messageOKU($persona), 200);
    }

    function ckeyLogin($codigoPersona, $key)
    {
        //buscamos persona        
        $persona = User::where('ncodpersona', $codigoPersona)->first();
